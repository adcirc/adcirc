...
```
Code must have a linear history to be merged into the main branch. If your branch becomes extremely out
...
4. Pull requests should be submitted to the `main` branch. Prior ADCIRC development has maintained a `main` and
   `development` branch, however, in practical use, only the `main` branch in addition to the release branches
   are necessary.
