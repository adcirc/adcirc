...
!       the time interpolation
  ...
  module hwrf_cap
    ...
    !To read grided netcdf file
    use hwrf_mod, only     : CreateGrid_ModelGrid
    use hwrf_mod, only     : read_hwrf_grid_nc
    use hwrf_mod, only     : UGRD10, VGRD10, PRMSL

    !To read mesh (unstructured) netcdf file
    use hwrf_mod, only     : init_hwrf_mesh_nc, read_hwrf_mesh_nc
    use hwrf_mod, only     : on_mesh_hwrf
    use hwrf_mod, only     : meshdata
    use hwrf_mod, only     : create_parallel_esmf_mesh_from_meshdata    
    use hwrf_mod, only     : UWND, VWND, PERS 
    !
    !
  ...
  !-----------------------------------------------------------------------------
   ...
   subroutine InitializeP1(model, importState, exportState, clock, rc)
    ...
    !initialze input files 
    if (on_mesh_hwrf) then
      call init_hwrf_mesh_nc()
    else
      FILE_NAME =  TRIM(wrf_dir)//'/hwrf_grid.nc'
      call read_hwrf_grid_nc()
    endif
  ...
  subroutine InitializeP2(model, importState, exportState, clock, rc)
        ...
        if (on_mesh_hwrf) then
         ...
         !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
          ...
          call HWRF_RealizeFields_grid(exportState, ModelGrid, fldsFrWrf_num, fldsFrWrf, "hwrf export", rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          ...
          call ESMF_MeshWrite(ModelMesh, filename="atm_mesh.nc", rc=rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
            line=__LINE__, &
            file=__FILE__)) &
            return  ! bail out
          ...
          print *,"NumOwnedNd mdataw", mdataw%NumOwnedNd
          !
          call HWRF_RealizeFields_mesh(exportState, ModelMesh, mdataw, fldsFrWrf_num, fldsFrWrf, "HWRF export", rc)
          if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU, &
              line=__LINE__, &
              file=__FILE__)) &
              return  ! bail out
...
!> Adds a set of fields to an ESMF_State object.  Each field is wrapped
  ...
  !! @param rc return code
  subroutine HWRF_RealizeFields_grid(state, grid, nfields, field_defs, tag, rc)
    ...
    integer                                     :: i
    character(len=*),parameter  :: subname='(hwrf_cap:HWRF_RealizeFields_grid)'
  ...
  end subroutine HWRF_RealizeFields_grid
  !--------------------------------------
 ...
 !> Adds a set of fields to an ESMF_State object.  Each field is wrapped
  ...
  !! @param tag used to output to the log
  !! @param rc return code
  subroutine HWRF_RealizeFields_mesh(state, mesh, mdata, nfields, field_defs, tag, rc)
    ...
    type(ESMF_Field)                            :: field
    integer                                     :: i
    character(len=*),parameter  :: subname='(HWRF:HWRF_RealizeFields_mesh)'
    ...
    enddo
        ...
        print *,      subname,' --- OUT --- '
        call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, line=__LINE__, file=__FILE__, rc=rc)
  end subroutine HWRF_RealizeFields_mesh
  !-----------------------------------------------------------------------------
