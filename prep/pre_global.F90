...
!                  For use with ADCPREP Version 2.2 (  09/20/2006 )

!                     current for ADCIRC v46.34   09/20/2006
!----------------------------------------------------------------------------
...
!     Land Boundary Nodes and Segment Definitions:
...
!                                    ! I-th Node on Land Boundary Segment K
...
!     jgf46.00 Nodal attributes section.
...
!   kmd48.33bc variables for 3D baroclinic and hot start
...
!     jgf45.12 Terms added for the transport parameters
!     diffusion coefficients for the salinity field
    REAL(SZ) :: NLSD, NVSD
!     diffusion coefficients for the temperature field
    REAL(SZ) :: NLTD, NVTD
!     time weighting coefficient for the vertical diffusion term in the
!     transport equation
    REAL(SZ) :: Alp4
!     Flag for the top boundary condition for the temperature field in
!     the transport equation
!      INTEGER :: NTF ! kmd48.33bc took out with new evaluation of boundary condition

!     STRING15-3DVS section
...
!  kmd48.33bc variables for boundary conditions
...
!     jgf45.11 Subdomain->Fulldomain element mappings for 3D recording stations
...
!...tcm v49.48.01 -- Added for new point in element searches
!-- KDTREE2 ENABLED SEARCH ALGORITHM VARIABLES
...
!     Allocate space for Arrays except those dimensioned by MNPP and MNEP
...
!....tcm 44.48.01 allocate memory for fast Search Tree
...
!     Allocate space for Arrays dimensioned by MNPP and MNEP
...
!--------------------------------------------------------------------------C
!                                                                          C
!                DEFINITIONS OF DOMAIN DECOMPOSITION VARIABLES             C
!                                                                          C
!--------------------------------------------------------------------------C

! Processing Element Definitions:

!   MNPROC               = Maximum Number of PEs
!   NPROC                = Actual Number of PEs - this should eventually be
!                          dropped and MNPROC used throughout preprocessor
!                          routines - RL
!   MSHAR                = Max. Number PEs assigned to any Global Node

! Nodal and Element Definitions:

!   X(I)                 = X-coordinate of Global Node I
!   Y(I)                 = Y-coordinate of Global Node I
!   DP(I)                = Bathymetry of Global Node I

!   NELG                 = Number of Global Elements
!   NELP(PE)             = Number of Elements Assigned to PE

!   NNODG                = Number of Global Nodes
!   NNODP(PE)            = Number of Nodes Assigned to PE

!   NOD_RES_TOT(PE)      = Number of Resident Nodes on PE
!   ITOTPROC(I)          = Number of PEs assigned to Global Node I

!   NNEG(3,I)            = Three Nodes of Global Element I
!   NNEP(3,I,PE)         = Three Nodes of Element I on PE

!   IMAP_NOD_GL(1,I)     = PE assigned to Global Node I
!   IMAP_NOD_GL(2,I)     = Local Node Number of Global Node I

!   IMAP_NOD_LG(I,PE)    = Global Node Number of Local Node I on PE
...
! Open Boundary Nodes and Segment Definitions:

!   NETA                 = Number of Global Open Boundary Nodes
!   NETAP(PE)            = Number of Open Boundary Nodes on PE

!   NOPE                 = Number of Global Open Boundary Segments
!   NOPEP(PE)            = Number of Open Boundary Segments on PE

!   NVDLL(K)             = Number of Nodes on Global Open Boundary Segment K
!   NVDLLP(K)            = Number of Nodes on Open Boundary Segment K on PE

!   NBDV(K,I)            = Global Node Number of I-th Node on Open Boundary
!                          Segment K
!   NBDVP(K,I)           = Local Node Number of I-th Node on Open Boundary
!                          Segment K on PE

!   OBNODE_LG(I,PE)      = Global Open Boundary Node Number of Local
!                          Open Boundary Node I on PE

! jp modified array to drop last dimension to save memory space
!   LBINDEX_LG(K,I,PE)   = Global Index of I-th Node on Land Boundary Segment
!                          K on PE

!   NWEIR                = Total Number of Global Weir Land Boundary Pairs
!   NWEIRP(PE)           = Total Number of Land Boundary Nodes on PE

!   WEIRP_LG(I,PE)       = Global Node Number of I-th Weir Node on PE
!   WEIRDP_LG(I,PE)      = Global Node Number of I-th Dual Weir Node on PE

! Elevation Station Definitions:

!   NSTAEP(PE)           = Number of Elevation Stations on PE
!   IMAP_STAE_LG(I,PE)   = Global Number of Local Elevation Station I on PE

! Velocity Station Definitions:

!   NSTAVP(PE)           = Number of Velocity Stations on PE
!   IMAP_STAV_LG(I,PE)   = Global Number of Local Velocity  Station I on PE

! Concentration Station Definitions:

!   NSTACP(PE)           = Number of Concentration Stations on PE
!   IMAP_STAC_LG(I,PE)   = Global Number of Local Concentration Station I on PE

! Meterological Station Definitions:

!   NSTAMP(PE)           = Number of Meterological Stations on PE
!   IMAP_STAM_LG(I,PE)   = Global Number of Local Meterological Station I on PE


! Message-Passing Definitions:

!   NUM_COMM_PE(PE)      = Number of PEs communicating with PE
!   COMM_PE_NUM(IPE,PE)  = IPE-th PE communicating with PE

!   IRECV_TOT(IPE,PE)    = Number of Nodes Received by PE from IPE
!   ISEND_TOT(IPE,PE)    = Number of Nodes Sent by PE to IPE

!   PROC_SV(PE)          = Surface-to-Volume Ratio on PE
