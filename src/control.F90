...
! check to see if we need to read in metadata
    ...
    outputSpec: do i=1,9
        metadataReq: do j=1,9
            if (outputSpecifiers(i) == metadataRequired(j)) then
                readMetaData = .TRUE. 
                exit outputSpec
            endif
        end do metadataReq
...
! initialization
    ...
    echoLine = echoLine + 1 ; if (echoLine > lineNum) return
