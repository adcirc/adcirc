...
#ifdef CMPI
      open (logUnit, FILE=trim(localdir)//'/'//'fort.16', ACTION='WRITE', &
            STATUS='REPLACE')
...
333            format(A, ": ", A, ": ", A)
   ...
   subroutine logMessage(level, message)
      ...
      if (level >= NABOUT) then
#ifdef FULL_STACK
         write (logUnit, 331, advance="no") trim(logLevelNames(level + 2)), &
            (trim(messageSources(j)), j=1, sourceNumber)
         write (logUnit, 332) trim(message)
331      format(A, ": ", A, 50(:, "->", A))
332      format(": ", A)
#else
         write (logUnit, 333) trim(logLevelNames(level + 2)), &
            trim(messageSources(sourceNumber)), trim(message)
333      format(A, ": ", A, ": ", A)
#endif
         flush (logUnit)
