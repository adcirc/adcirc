...
!---------------------------------------------------------------------
!  Update 1, 2, or 3 Integer Arrays's Ghost Cells using asynchronous
!  and persistent message-passing.
...
! equals the total number that were requested
    ...
    DO WHILE (TOT < RDIM)
        ...
        TOT = TOT + NFINI
    ! ebug     print *, myproc, tot,nfini,INDX(1),INDX(2)
        DO N=1, NFINI
            IF (INDX(N) > 0 .AND. INDX(N) <= RDIM)  THEN
                IF (INDX(N) <= NEIGHPROC) THEN
                    J = INDX(N)
...
! Send/receive messages to/from all neighbors
    ...
    !     jgf48.02 Commented out the following two lines and added the
    !     two after that to fix bug.
    !         CALL MPI_SEND_INIT ( SENDBUF(1,J), 2*MNFEN*NNODSEND(J),
