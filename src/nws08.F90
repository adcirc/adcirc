...
contains
   ...
   integer function getVortexModelId(vortexModelStr) result(id)
      ...
      case default
         write (scratchMessage, '(a,a)') "Unrecognized vortex model: ", &
            trim(vortexModelStr)
   ...
   !----------------------------------------------------------------
   character(20) function getVortexModelString()
   ...
   integer function getBackgroundWindModelId(backgroundWindModelStr) result(id)
      ...
      case default
         write (scratchMessage, '(a,a)') "Unrecognized background wind model: ", &
   ...
   !----------------------------------------------------------------
   character(20) function getBackgroundWindModelString()
      use global, only: scratchMessage, allMessage, ERROR
      implicit none
   ...
   integer function getBCalcId(BCalcStr) result(id)
      ...
      case ("limited")
         id = BCALC_LIMITED
      case default
         write (scratchMessage, '(a,a)') "Unrecognized BCalc: ", &
            trim(BCalcStr)
         call allMessage(ERROR, trim(scratchMessage))
         call nws08terminate()
      end select
   ...
   !----------------------------------------------------------------
   character(20) function getBCalcString()
   ...
   subroutine readNws08Namelist(iounit)
      ...
      integer :: ios, io_stat
      character(len=20)  :: vortexModel
      character(len=20)  :: backgroundWindModel
      character(len=20)  :: BCalc
...
! ----------------------------------------------------------------
   subroutine NWS08INIT(timeloc)
      ...
      call setMessageSource("NWS8INIT")
...
4321  format(/, 2x, 'Holland B parameter is ', e16.8)
      ...
      do I = 1, NP
         ...
         V_r(I) = sqrt( &
                  alpha*exp(1.d0 - alpha)*spd*(spd + non_cyclostrophic_part) &
