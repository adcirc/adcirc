!******************************************************************************
!  MODULE OWI_ICE
!    Written by Chris Massey, USACE-ERDC-CHL 12/15/2010
!    Modified from owiwind.F Written by s.b. 08/17/2006

!    The values read in from the fort.225/fort.227 contain ice percent
!    concentration values that range from 0.0 to 100.0 percent.  A value
!    of -1.0 is used to signal that either the cell is over land, or no ice
!    data was available.
...
! Skip snaps if necessary ---------------------------------------------------
    ...
    call msg_fini()
#endif
    STOP 'OWI_ICE READ ERROR (1)'

    END SUBROUTINE

!***********************************************************************
!   SOBROUTINE NCICE1_GET
!***********************************************************************
...
! Check if header info has changed from the previous snapshot
    if(isnapB > 1) then
        if(iLatc /= iLatB .OR. iLongc /= iLongB .OR. dxc /= dxB .OR. &
        dyc /= dyB .OR. swlatc /= swlatB .OR. &
        swlongc /= swlongB) then
            updateB = 1
        else
            updateB = 0
        endif
...
! Check if header info has changed from the previous snapshot
    if(isnapR > 1) then
        if(iLatc /= iLatR .OR. iLongc /= iLongR .OR. dxc /= dxR .OR. &
        dyc /= dyR .OR. swlatc /= swlatR .OR. &
        swlongc /= swlongR) then
            updateR = 1
        else
            updateR = 0
        endif
...
! Read ice concentrations
    read(223,22,end=9999,err=99999) ((CiceR(i,j),i=1,iLongR),j=1,iLatR)
...
!      end if
    ...
    ! BASIN ---------------------------------------------------------
        ...
        !         interpolating
            ...
            enddo
            if (ibar > 0) CVbar = CVbar/real(ibar,sz)
            do k=1,4
                if (CV(k) < 0.d0) CV(k) = CVbar
            enddo
...
!***********************************************************************
!   SUBROUTINE NCICE1_INTERP_BASIN
...
!***********************************************************************
!   SUBROUTINE NCICE1_INTERP_REGION
