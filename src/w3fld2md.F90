...
!/    16-May-2014 : Finalizing                          (version 4.xx)
!/    19-Mar-2015 : Extending for non-10 m winds        (version 5.xx)
!/
...
!/
!/    01-Jul-2013 : Origination                         (version 3.14)
!/    19-Mar-2015 : Clean-up for submission             (version 5.xx)
!/
!  1. Purpose :
!
...
!/      USE W3SERVMD, ONLY: EXTCDE
      ...
      USE SWCOMM3, ONLY       : GRAV, PWIND, PI2, PI, &
                                MSC, MDC, DDIR !, MMCGR
      USE M_GENARR, ONLY      : SPCDIR, SPCSIG

      USE W3FLD1MD, ONLY: APPENDTAIL,sig2wn, wnd2z0m,mflux
!/
      IMPLICIT NONE
!/
!/ ------------------------------------------------------------------- /
...
!/
      ...
      !-Stress Calculation
      INTEGER :: K, T, ITS
      REAL :: TAUXW, TAUYW, TAUXV, TAUYV !, TAUX, TAUY
...
!      END IF
      ...
      allocate(SIG(NK))
      SIG = SPCSIG ! relative frequency in radian!!!
      XFR = SIG(NK)/SIG(NK-1)     
      allocate(TH(NTH))
      DO T = 1, NTH 
        TH(T) = SPCDIR(T,1)
      ENDDO 
       ...
       !debug only 
       !write(16,*) 'NSPEC=',NSPEC,'NK=',NK,'DTH=',DTH,'XFR=',XFR   
...
!/
      ...
      DO WHILE ( WIFLG .AND. NO_ERR )  !Wind iteration
         ...
         DO WI = 1, ITTOT   !Newton-Raphson solve for derivatives if zwnd not 10 m.
            ...
            DO WHILE ((UST_IT_FLG(1) .AND. UST_IT_FLG(2)) .AND. NO_ERR)
               ...
               TAUXV = DAIR * CDS * wnd_10_mag**2.0 * COS( wnd_10_dir )
               TAUYV = DAIR * CDS * wnd_10_mag**2.0 * SIN( wnd_10_dir )
               !-----Sum drag components
               TAUX = TAUXV + TAUXW
               TAUY = TAUYV + TAUYW
               !-----Calculate USTAR
               ...
               ELSEIF (ABS(B1*100.0/B2) .GE. 5.) THEN
                  ...
                  UST_IT_FLG(1) = .FALSE.
                  UST_IT_FLG(2) = .FALSE.
                  write(16,*) 'Attn: W3FLD2 not converged for windspeed: ',UREF, 'At water depth=',DEPTH 
                  UST = -999.
                  NO_ERR = .false.
               ENDIF
            ENDDO
         ...
         IF (HeightFLG) THEN
            ...
            ELSE
               ...
               IF (WI_COUNT .gt. 10 .AND. &
                  ...
                  write(16,*) 'Attn: W3FLD2 Wind error gt 5%'
      ...
      IF (.not.((CD .LT. 0.005).AND.(CD .GT. 0.0005)).or. .not.(NO_ERR)) THEN
         !Fail safe to bulk
         write(16,*) 'Attn: W3FLD2 failed, using bulk stress'
         write(16,*) '      Wind/Cd that failed: ',WND_10_mag,CD
      ...
      DEALLOCATE( TAUINTY , TAUINTX , &
                  SPC2, sig2, CP , DWN , WN, SIG, TH) !jie 08/2015
      !STOP
