!----------------------------------------------------------------------
!      M O D U L E    W R I T E   O U T P U T
!----------------------------------------------------------------------
! jgf48.03 This module contains all the subroutines that write output to
! files. It was created to consolidate all the output routines that
! were located in other parts of the code, to make it easier to incorporate
! them into globalio, and to make it easier to add new output modes or
! formats in the future.

! tcm48.4618 Fixed Bug in writing out global elevation values that are
! dry.
!----------------------------------------------------------------------
    MODULE WRITE_OUTPUT
!----------------------------------------------------------------------
...
#ifdef CSWAN
    integer, parameter :: numOutputDescript2D = 45
#else
    integer, parameter :: numOutputDescript2D = 31
#endif
...
! cm v49.64.01 added for ice
    type(OutputDataDescript_t), target :: IceDescript
! cm v49.64.01 added for ice stations
...
! cm v50.66.01 added time varying bathy
    type(OutputDataDescript_t), target :: BathyDescript
! cm v50.66.01 added time varying bathy stations
...
! bell Added a descript for time varying weir tracking
    type(OutputDataDescript_t), target :: weirElevDescript

!     jgf48.03 Full domain arrays used when writing globalio output.
...
! jgf52.05: Added for normal output of wet/dry quantities.
...
!     Fulldomain Stations in parallel:
...
!     Fulldomain Time means and variance calculation in parallel:
...
!----------------------------------------------------------------------
!     jgf51.21.24: Initialize 2D output data structures.
!----------------------------------------------------------------------
    ...
    xem, yem, slem, sfem, rmu00, rmv00, wvnxout, wvnyout, &
    ...
    inundationOutput, ihot, logMessage
...
#ifdef CSWAN
    ptr(32)%descript2D => SwanHSDescript
    ptr(33)%descript2D => SwanHSMaxDescript
    ptr(34)%descript2D => SwanDIRDescript
    ptr(35)%descript2D => SwanDIRMaxDescript
    ptr(36)%descript2D => SwanTM01Descript
    ptr(37)%descript2D => SwanTM01MaxDescript
    ptr(38)%descript2D => SwanTPSDescript
    ptr(39)%descript2D => SwanTPSMaxDescript
    ptr(40)%descript2D => SwanWindDescript
    ptr(41)%descript2D => SwanWindMaxDescript
    ptr(42)%descript2D => SwanTM02Descript
    ptr(43)%descript2D => SwanTM02MaxDescript
    ptr(44)%descript2D => SwanTMM10Descript
    ptr(45)%descript2D => SwanTMM10MaxDescript
#endif
...
! jgf51.21.24: Set some defaults
    ...
    ! jgf52.08.03: initiallydry.63 already initialized and written by now
        if (trim(ptr(i)%descript2D%field_name) /= 'initiallyDry') then
...
! bathy, even if they were specified for fulldomain elevation file
    if ((abs(noutge) == 4) .OR. (abs(noutge) == 7)) then
...
! fort.93
!         write(16,*) 'Allocating IceDescript' !tcm v49.64.01 added for ice
    IF ( (MNPROC > 1) .AND. (MyProc == 0) ) THEN
...
!... v49.64.01 tcm -- added for ice stations
! fort.91
    IF ( (MNPROC > 1) .AND. (MyProc == 0) ) THEN
...
! rads.64
! Radiation Stress
    IF ( (MNPROC > 1) .AND. (MyProc == 0) .AND. (nrs /= 0) ) THEN
...
!     maxele.63
    ...
    if ( EtaMaxDescript % specifier == XDMF ) then
...
!     maxvel.63
    ...
    if ( UMaxDescript % specifier == XDMF ) then
...
!     prmin.63
    ...
    if ( PrMinDescript % specifier == XDMF ) then
...
!     maxwvel.63
    ...
    if ( WVMaxDescript % specifier == XDMF ) then
...
!     maxrs.63
    ...
    if ( RSMaxDescript % specifier == XDMF ) then
...
! inundationtime.63 (works like a min/max file)
    ...
    if ( InundationTimeDescript % specifier == XDMF ) then
...
! maxinundepth.63 (works like a min/max file)
    ...
    if ( MaxInunDepthDescript % specifier == XDMF ) then
...
! write_flag set to .false. immediately after file initialization
    if ( EndRisingInunDescript % specifier == XDMF ) then
...
! to overwrite.
    if ( EndRisingInunDescript % specifier > 0 ) then
...
! everdried.63 (works like a min/max file)
    ...
    if ( EverDriedDescript % specifier == XDMF ) then
...
! create file names
    ...
    ! in the code above
        if ( ptr(i) % descript2D % file_extension == -99999 ) then
    ...
    ! jgf: netCDF and XDMF files are always fulldomain...
        ...
        if (((mnproc > 1) .AND. (WRITE_LOCAL_FILES.eqv. .FALSE. )) .OR. &
        (ptr(i) % descript2D % specifier == NETCDF3) .OR. &
        (ptr(i) % descript2D % specifier == NETCDF4) .OR. &
        (ptr(i) % descript2D % specifier == XDMF) ) then
    ...
    if (mnwproc > 0) then
        ...
        ! this output type
            if ( ptr(i) % descript2D % writeFlag .eqv. .TRUE. ) then
                if ( any(ptr(i) % descript2D % writerFormats(:) == &
...
! current adcirc time.
    if (ihot /= 0) then
            ...
            if ( ptr(i) % descript2D % readMaxMin.eqv. .TRUE. ) then
                ...
                ! initialize it
                    ...
                    if (fileFound.eqv. .TRUE. ) then
                        nerr = .FALSE. 
                        if (myproc == 0) then
                        !                      ! initialize it if it wasn't initialized above
        ...
        ! attribute, and only turning on the slope limiter there.
            DO I = 1,NP
                if ( (ESLONOFF(I) == 1.d0) .AND. &
                (elemental_slope_limiter_grad_max(i) >= 0.d0) ) then
...
!     R.L. 8/22/05 Subroutine to write primary 2D model output not
!     including hotstart and harmonic analysis.
...
!----------------------------------------------------------------------
    ...
    ! if this output was not selected in the input file, go to the next one
        if (ptr(i) % descript2D % specifier == OFF) then
            cycle
        endif
    
    ! if we are in the time step window where output should be produced,
    ! increment the spool counter for this output type
        if ( (it > ptr(i) % descript2D % startTimeStep) .AND. &
        (it <= ptr(i) % descript2D % endTimeStep) ) then
    ...
    ! the next data type
        if (ptr(i) % descript2D % spoolCounter /= &
    ...
    !  W R I T E   U S I N G   D E D I C A T E D   W R I T E R
        ...
        if ( ptr(i) % descript2D % useWriter .eqv. .TRUE. ) then
            if ( ptr(i) % descript2D % num_items_per_record == 1 ) then
#ifdef CMPI
    ...
    ! sponge only outputs once
        if ( ptr(i) % descript2D % lun == 92 ) then
...
!...  they have.
! rite(6,'(a)') 'DEBUG: Calling flush_writers()'
    CALL FLUSH_WRITERS()
#endif

! b 11/11/2006
!...   OUTPUT MAXIMUM/MINIMUM GLOBAL DATA AT THE LAST TIME STEP.
!...
!     jgf48.4636 Updated to newer data structure, so that we can
!     compare with values from hot start, so that mins and maxes
!     can be preserved across hot starts.
!     tcmv51.20.01 Changed from pack/unpack one to pack/unpack two
!       as max/min are now initialized from file during hotstart
!       for all processors, not just myproc=0.
    if (it == nt) then
...
!-----------------------------------------------------------------------
    ...
    descript % array)
    if ( descript % num_items_per_record == 2 ) then
...
!----------------------------------------------------------------------
!       S U B R O U T I N E
!          W R I T E  H A R M O N I C   A N A L Y S I S   O U T P U T
!----------------------------------------------------------------------
!     jgf49.44: Subroutine to write harmonic analysis output files
!     in serial or in parallel using globalio.
!----------------------------------------------------------------------
...
! RJW added MNPROC and MYPROC
! removed from global  myProc, MNPROC,
...
! consistent with what is in solveHarmonicAnalysis
    IF ((IHARIND == 0) .OR. (ITIME <= ITHAS)) THEN
#if defined(WRITE_OUTPUT_TRACE) || defined(ALL_TRACE)
        call allMessage(DEBUG,"Return.")
#endif
        RETURN
...
!     This subroutine should only be called once, at the end of a run, so
!     a check of FirstCall is not necessary. However, we'll still do it, in
!     case the subroutine is ever called more than once in some future
!     scenario.
    IF (FirstCall) THEN
    ...
    ! instead of building fn within writeOutHarmonicArrays() subroutine
        ...
        ! fort.51 magnitude
            ...
            IF ((MNPROC > 1) .AND. (WRITE_LOCAL_HARM_FILES.eqv. .FALSE. )) THEN
                IF (MyProc == 0) THEN
    ...
    !        ! fort.55 means and variance calculations

    !        ! fort.55 will use the same output format (ascii, netcdf, etc)
    !        ! as the full domain elevation harmonic analysis file (fort.53)
...
!     Collect fulldomain data if running in parallel, and we are not
!     writing subdomain harmonic analysis output files.
#ifdef CMPI
    ...
    if ((CHARMV.eqv. .TRUE. ) .AND. (FMV > 1.0d-3)) then
        IF ((MNPROC > 1) .AND. (MyProc == 0) &
         .AND. (WRITE_LOCAL_HARM_FILES.eqv. .FALSE. )) THEN
            ...
            CLOSE(55)
        ENDIF
        IF ((MNPROC == 1) .OR. (WRITE_LOCAL_HARM_FILES.eqv. .TRUE. )) THEN
...
!-----------------------------------------------------------------------
!     S U B R O U T I N E
!            W R I T E   O U T   H A R M O N I C  A R R A Y S
!-----------------------------------------------------------------------
!     jgf49.44 Writes the harmonic arrays in output files of the desired
!     format. Only ascii is supported at this point.
!-----------------------------------------------------------------------
...
!     write data according to format specifier from fort.15 (e.g., NHASE)
...
!----------------------------------------------------------------------
!       S U B R O U T I N E   I N I T   O U T P U T   3 D
!----------------------------------------------------------------------
!     jgf49.43.19:  Subroutine to initialize ascii or nonportable binary
!     output for a 3D run.
!----------------------------------------------------------------------
...
!     form file name from lun
    fn(1:5) = 'fort.'
    WRITE(fn(6:7),'(I2)') lun

    IF ((specifier == -1) .OR. ((specifier == 1) .AND. (IHOT == 0))) THEN
    ! tart a new ASCII file
        ...
        ENDIF
    ENDIF
    IF ((specifier == 1) .AND. (IHOT /= 0)) THEN
    ! find spot in existing ASCII file
        ...
        ENDIF
    ENDIF
    IF ((specifier == -2) .OR. ((specifier == 2) .AND. (IHOT == 0))) THEN
    ! tart a new BINARY file
        WRITE(16,496) lun
        ...
        CLOSE(lun)
    ENDIF
    IF ((specifier == 2) .AND. (IHOT /= 0)) THEN
    ! ind spot in existing BINARY file
        WRITE(16,496) lun
        OPEN(lun,FILE=TRIM(LOCALDIR)//'/'//fn,ACCESS='DIRECT', &
        RECL=NByte)
        CLOSE(lun)
...
!********************************************************
!  Subroutine to generate 3D Model Output


!  Note, the FORM='FORMATTED' field in the binary open statements
!  means that the record length (RECL) is specified in bytes.
!  If this field is omitted, the record length must be specified
!  in 4 byte words.

!  r.l.  8/23/2005
!*********************************************************
!-----------------------------------------------------------------------
!     S U B R O U T I N E   W R I T E  O U T P U T  3 D
!-----------------------------------------------------------------------
!     jgf48.11 Moved from vsmy.F to write_output.F.
!     jgf49.43 Rewrote for globalio.
!-----------------------------------------------------------------------
...
!.... 3D Density, Temperature, Salinity Station Output (Unit 41)

    IF(I3DSD /= 0) THEN
    !        Check to see if it is time to generate 3D station density output.
        IF((IT > NTO3DSDS) .AND. (IT <= NTO3DSDF)) N3DSD=N3DSD+1
        IF(N3DSD == NSpo3DSD) THEN
        ...
        ! jgf49.43.19 Collect fulldomain data in parallel.
            IF ((MNPROC > 1) .AND. (WRITE_LOCAL_FILES.eqv. .FALSE. )) THEN
                IF (myProc == 0)  THEN
                ...
                IF ((ABS(IDEN) == 3) .OR. (ABS(IDEN) == 4)) THEN
                    ...
                    ! md : Took out these two lines as already allocated above
                    ! LLOCATE(SalSta_g(NSTA3DD_G,NFEN))
                    ! alStaDescript % array2D_g => SalSta_g
                        TempStaDescript % array2D_g => TempSta_g
                    ENDIF
        ...
        ! deallocate memory used in globalio
            ...
            (myProc == 0)) THEN
                ...
                ! md : Changed Deallocate line above as Salinity is already deallocated
                ! EALLOCATE(TempSta_g,SalSta_g)
                ENDIF
            ENDIF
...
!.... 3D Velocity Station Output (Unit 42)

    IF(I3DSV /= 0) THEN
    !        Check to see if it is time to generate 3D station velocity output.
        IF((IT > NTO3DSVS) .AND. (IT <= NTO3DSVF)) N3DSV=N3DSV+1
        IF(N3DSV == NSpo3DSV) THEN
        !           Process each station
            DO NN=1,NSta3DV
            !              Interpolate to station locations
                NEle=NE3DV(NN)
...
!.... 3D Turbulence Station Output (Unit 43)

    IF(I3DST /= 0) THEN
    !        Check to see if it is time to generate 3D station turbulence output.
        IF ((IT > NTO3DSTS) .AND. (IT <= NTO3DSTF)) N3DST=N3DST+1
        IF (N3DST == NSpo3DST) THEN
        !           Process each station
            DO NN=1,NSta3DT
            !              Interpolate to station locations
                NEle=NE3DT(NN)
...
!.... 3D Density, Temperature, Salinity Global Output (Unit 44)

    IF(I3DGD /= 0) THEN
        IF((IT > NTO3DGDS) .AND. (IT <= NTO3DGDF)) N3DGD=N3DGD+1
    !        Check to see if it is time to generate 3D fulldomain density output.
        IF(N3DGD == NSpo3DGD) THEN
        ! collect up output data in parallel if fulldomain files are needed
            ...
            ! already collected SigT above
                ...
                END SELECT
            ENDIF
        ! md48.33bc - add in the information for outputting the top temperature
        !             boundary condition.
...
!.... 3D Velocity Global Output (Unit 45)

! kmd48.33bc changed to NE from GT
    IF(I3DGV /= 0) THEN
        IF ((IT > NTO3DGVS) .AND. (IT <= NTO3DGVF)) N3DGV=N3DGV+1
    !        Check to see if it is time to generate 3D fulldomain velocity output.
        IF (N3DGV == NSpo3DGV) THEN
            rp = real(q)
            ip = aimag(q)
            IF ((MNPROC > 1) .AND. (WRITE_LOCAL_FILES.eqv. .FALSE. )) THEN ! parallel
                IF (myProc == 0) THEN
...
!.... 3D Turbulence Global Output (Unit 46)

! kmd48.33bc changed to NE from GT
    IF(I3DGT /= 0) THEN
    !        Check to see if it is time to generate 3D fulldomain turbulence output.
        IF((IT > NTO3DGTS) .AND. (IT <= NTO3DGTF)) N3DGT=N3DGT+1
        IF (N3DGT == NSpo3DGT) THEN
            IF ((MNPROC > 1) .AND. (WRITE_LOCAL_FILES.eqv. .FALSE. )) THEN
                IF (myProc == 0) THEN
...
!     jgf51.21.24: Updated arguments to take into account the more
!     comprehensive information found in the OutputDataDescript_t.
!-----------------------------------------------------------------------
...
!     write data according to format specifier from fort.15 (e.g., NOUTE)
    ...
    IF ( (MNPROC > 1) .AND. (MyProc == 0) &
     .AND. ( .NOT. WRITE_LOCAL_FILES)) THEN
        ...
        IF (descript % num_items_per_record == 1) THEN
            DO I=1, descript % num_fd_records
                if ( descript % isInteger .eqv. .TRUE. ) then
                    write(descript % lun,2452) I, descript % iarray_g(I)
                else
        ...
        IF (descript % num_items_per_record == 2) THEN
            DO I=1, descript % num_fd_records
                WRITE(descript % lun,2454) I, descript % array_g(I), &
                descript % array2_g(I)
            ENDDO
        ENDIF
        CLOSE(descript % lun)
        ...
        !......TCM - v48.4618 -- Fixed dry node output for serial run case
            IF (descript % ConsiderWetDry .EQV. .TRUE. ) THEN
                ...
                ! above. For stations, it doesn't make sense to check the nodecode.
                    if ( (descript%isStation.eqv. .FALSE. ) .AND. (NODECODE(I) == 0)) then
                        if ( descript % isInteger .eqv. .TRUE. ) then
                            write(descript % lun,2452) I, int(descript % alternate_value)
                        else
                            write(descript % lun,2453) I, descript % alternate_value
                        endif
                    else
                        if ( descript % isInteger .eqv. .TRUE. ) then
    ...
    IF ((MNPROC == 1) .OR. (WRITE_LOCAL_FILES)) THEN
        ...
        descript % filepos = descript % filepos + 2
        IF ( descript % num_items_per_record == 1 ) THEN
            IF ((trim(descript % field_name) == 'Elev') .AND. &
        ...
        IF ( descript % num_items_per_record == 2 ) THEN
            DO I=1, descript % num_records_this
            ! cmv48.4618 -- changed from array_g to array
                WRITE(descript % lun,REC=descript % filepos+2*I-1) descript % array(I)
            ! cmv48.4618 -- changed from array2_g to array2
...
!-----------------------------------------------------------------------
!     S U B R O U T I N E   W R I T E  O U T  A R R A Y  M I N  M A X
!-----------------------------------------------------------------------
!     jgf48.4636 This subroutine was created to write out a column
!     vector (i.e., nodal data such as water surface elevation or
!     pressure) to a file, for quantities whose min or max values
!     have been stored over the course of the run.
!-----------------------------------------------------------------------
...
#endif
!     collect up the data from subdomains if running in parallel
    IF ((MNPROC > 1) .AND. ( .NOT. WRITE_LOCAL_FILES)) THEN
        CALL collectFullDomainArray(descript, pack_cmd, unpack_cmd)
    ENDIF

!     write data in ascii text only (other output formats are not supported)
    SELECT CASE (ABS(descript % specifier))
    ...
    ! cm v51.20.01 additions for time stamp output
        if (descript%minmax_timestamp) then
            WRITE(lun,2120) TimeLoc,IT
            DO I=1, descript % num_fd_records
                WRITE(lun,2453) I, descript % array2_g(I)
            ENDDO
        endif
    ...
    ! cm v51.20.01 additions for time stamp output
        if (descript%minmax_timestamp) then
            WRITE(lun,2120) TimeLoc,IT
            DO I=1, descript % num_records_this
                WRITE(lun,2453) I, descript % array2(I)
            ENDDO
        endif
...
!-----------------------------------------------------------------------
!        S U B R O U T I N E    W R I T E   H O T S T A R T
!-----------------------------------------------------------------------
!     jgf48.03 This subroutine was created from code in timestep.F to
!     write hotstart files.
!-----------------------------------------------------------------------
...
!     !jgf49.44: harmonic analysis vars
    ...
    IF (FirstCall) THEN
        ...
        ! created by adcprep.
            ...
            !              jgf49.35 TODO: Allocate arrays to write 3D data
...
!     collect up the data from subdomains if running in parallel
    ...
    !        Collect harmonic analysis data if needed.
        IF ((IHARIND == 1) .AND. (IT > ITHAS)) THEN
            IF (NHAGE /= 0) THEN
    ...
    !        Collect timeseries reconstruction data if needed.
        IF (CHARMV) THEN
            IF ((IHARIND == 1) .AND. (IT > ITMV)) THEN
...
! cm v51.26 mod for time-stamped nhstar=-1
    CASE(-1,1,67,68) ! 1=nonportable binary
!        jgf49.35 Values of 67 and 68 are used for testing and cause the
!        program to exit immediately after writing the hotstart file.
    NPX = NP_G
    NEX = NE_G
    if ((MNPROC == 1) .OR. &
    ...
    ENDIF
!...
!...  ADD IN 3D STUFF IF APPROPRIATE
!...
...
!...
    IF((IHARIND == 1) .AND. (IT > ITHAS)) THEN
        IF ((MNPROC == 1) .OR. (WRITE_LOCAL_HOT_START_FILES.eqv. .TRUE. )) THEN
            NSTAEX = NSTAE
            NSTAVX = NSTAV
        ELSE
            NSTAEX = NSTAE_G
    ...
    ! will write these data to subdomain hotstart files.
        IF ((MyPROC == 0) .OR. (WRITE_LOCAL_HOT_START_FILES.eqv. .TRUE. )) THEN
    ...
    if ((CHARMV.eqv. .TRUE. ) .AND. (IHARIND == 1) .AND. (IT > ITMV)) then
        if ( (MNPROC == 1) .OR. &
        (WRITE_LOCAL_HOT_START_FILES.eqv. .TRUE. )) then
...
!        jgf49.35 write both subdomain and fulldomain hotstart files,
!        just to have them, since this is just for debugging purposes
!        (i.e., to check the values written to hotstart files in other
!        formats) anyway.
...
!        TODO: 3D data in ascii
!        TODO: harmonic analysis data in ascii
    ...
    TEST_HOTSTART = .FALSE. 
    IF(hss % lun == 67) THEN
    !        jgf45.07 added option to stop ADCIRC after writing hot start file.
        IF ((NHSTAR == 67) .OR. (NHSTAR == 267) .OR. (NHSTAR == 367) .OR. &
        (NHSTAR == 567)) THEN
            call allMessage(INFO,"NHSTAR == 67; ADCIRC stopping.")
            TEST_HOTSTART = .TRUE. 
        ENDIF
        hss % lun      = 68
        IF((NHSTAR == 1) .OR. (NHSTAR == 2)) THEN
            hss % filename(:) = ' '
            hss % filename = 'fort.68'
        ENDIF
    ! cm v51.26 mod for time-stamped nhstar=-1  This section is just a failsafe
        IF (NHSTAR == -1) THEN
            hss % filename(:) = ' '
            hss % filename = 'fort.68_'
            itstr(:) = ' '
        ...
        ENDIF
    ELSE
        IF ((NHSTAR == 68) .OR. (NHSTAR == 268) .OR. (NHSTAR == 368) .OR. &
        (NHSTAR == 568)) THEN
            call allMessage(INFO,"NHSTAR == 68; ADCIRC stopping.")
            TEST_HOTSTART = .TRUE. 
        ENDIF
        hss % lun=67
        IF((NHSTAR == 1) .OR. (NHSTAR == 2)) THEN
...
!-----------------------------------------------------------------------
!     S U B R O U T I N E   W R I T E  W A R N  E L E V
!-----------------------------------------------------------------------

!     jgf46.10 Subroutine write an elevation file to fort.69 if any
!     elevation exceeds the WarnElev and the user has set WarnElevDump
!     to .True. (1 in the fort.15 input file). Terminate the run
!     if the number of dumps to the fort.69 file exceeds the user's
!     limit of WarnElevDumpLimit.
...
!     Open the file and write the header on the first warning. Format
!     the actual header data to make it look more or less like a fort.63
!     file.
    IF (WarnElevDumpCounter == 0) THEN
...
!     Open the file and append the new elevation data.
    OPEN(69,FILE=TRIM(LOCALDIR)//'/'//'fort.69', &
    ACCESS='SEQUENTIAL',POSITION='APPEND')
...
!     If we have exceeded the user's limit, terminate the run.
    IF (WarnElevDumpCounter > WarnElevDumpLimit) THEN
...
!-----------------------------------------------------------------------
!     S U B R O U T I N E  i n i t S w a n O u t p u t
!-----------------------------------------------------------------------
! THIS ROUTINE WILL PERFORM THE ASSIGNMENT AND ALLOCATIONS FOR THE
! SWAN OUTPUT VARIABLES USED IN WRITE_OUTPUT.F
!-----------------------------------------------------------------------
#ifdef CSWAN
...
!.....Allocate SWAN specific local/global arrays if that output is selected
    IF(SWAN_OutputHS)THEN
        IF(MYPROC == 0 .AND. MNPROC > 1)THEN
...
!-----------------------------------------------------------------------
!      S U B R O U T I N E   I N I T   I N U N D A T I O N   O U T P U T
!-----------------------------------------------------------------------
!     jgf52.08.01: Initialize output variables and arrays for detailed
!     inundation output. These variables must be initialized prior
!     to the call to initOutput2D() because the initiallyDry array
!     depends on the coldstart values in the nnodecode array, which
!     are not available after the call to hstart().

!     The initiallydry.63 file must be written during the initialization
!     process (coldstart and hotstart), rather than waiting for the first
!     call to writeOutput2D().
!-----------------------------------------------------------------------
...
! initiallydry.63 (nodes that are deemed dry upon cold start)
    ...
    if ( InitiallyDryDescript % specifier == XDMF ) then
...
!-----------------------------------------------------------------------
!                     S U B R O U T I N E
!         C O L L E C T   I N U N D A T I O N   D A T A
!-----------------------------------------------------------------------
!     jgf52.08.01: Collect data used for inundation output.
!-----------------------------------------------------------------------
...
! threshold to the total water depth.
    where ( (initiallyDry == 1) .AND. (nodecode == 1) .AND. &
        ((eta2+dp) >= inunThresh) )
...
! on the very last time step, just in case.
    if (it == nt) then
        where ( (initiallyDry == 1) .AND. (penultimateNodeCode == 1) &
             .AND. (nodecode == 1) .AND. (eta2 > eta1) )
            endRisingInun = 1
        end where
    endif
...
!-----------------------------------------------------------------------
!                     S U B R O U T I N E
!            C O L L E C T   M I N   M A X   D A T A
!-----------------------------------------------------------------------
!     jgf52.08.01: Collect data used for min/max output. Removed
!     from subroutine timestep() and placed here.
!-----------------------------------------------------------------------
    subroutine collectMinMaxData(timeloc)
    use global, only : nodecode, eta2, uu2, vv2, pr2, wvnxout, wvnyout, &
...
!...  jgf50.44: Collect time that min/max occurs as well.
    where ( (nodecode == 1) .AND. (eta2 > etamax) )
        etamax = eta2
        etamax_time = timeLoc !tcm v51.20.01
    end where
          
    minMaxTemp = uu2**2+vv2**2
    where ( minMaxTemp > umax**2 )
