!----------------------------------------------------------------------
!                     M O D U L E     X D M F I O
!----------------------------------------------------------------------
!     jgf51.21.28: Created to hold XDMF-related output routines
!     where they can be called by the writer module as well as the
!     write output module.
!----------------------------------------------------------------------
    module xdmfio
!----------------------------------------------------------------------
    ...
    contains
!-----------------------------------------------------------------------
!     S U B R O U T I N E   I N I T  O U T P U T   X D M F
!-----------------------------------------------------------------------
!     Writes the ADCIRC mesh to the XDMF output file.
!-----------------------------------------------------------------------
...
! for this output file type
    if (abs(descript%specifier) /= XDMF) then
        return
    endif
#ifndef ADCXDMF
...
! all readControlFile(controlFileName)
! f (nabout.eq.-1) then
...
! all writeControlXDMF(xdmfFortranObj)
...
! the first Grid, then just add references to the mesh and boundaries
    ...
    ! escript % xdmfmd % createdIDs = .false.
    ! escript % xdmfmd % variable_name = 'depth'
    ! escript % xdmfmd % long_name = 'distance from geoid'
    ! escript % xdmfmd % standard_name = 'depth_below_geoid'
    ! escript % xdmfmd % coordinates = 'y x'
    ! escript % xdmfmd % units = 'm'
    ! escript % xdmfmd % positive = 'downward'
    ! all writeMetaData(xdmfFortranObj, md)
    ! write projection info
    ! d%variable_name_id = XdmfAddInformation(xdmfFortranObj,
    ...
    ! all xdmfSetPreviousTopology(xdmfFortranObj, topologyID)
    ! all xdmfSetPreviousGeometry(xdmfFortranObj, geometryID)
...
! modify projection string if indicated by the fort.15
    select case(ics)
    case(1)
    projection = 'cartesian' ! most likely CPP
    case(2)
...
!----------------------------------------------------------------------
    ...
    if (descript % xdmfMD % createdIDs.eqv. .TRUE. ) then
        ...
        if (trim(descript % xdmfMD % positive) /= 'null') then
            call XdmfAddPreviousInformation( &
            descript % xdmfMD % xdmfFortranObj, &
...
!---------------------------------------------------------------------
    ...
    trim(adjustl(info))//char(0))
    if ((tau0 <= -5.0) .AND. (tau0 >= -5.99)) then
...
! do nothing
    ...
    if ((nws /= 0) .OR. (nrs /= 0) .OR. (ncice /= 0)) then
        informationID = XdmfAddInformation(xdmfFortranObj,trim(wtimincComment)//char(0), &
        trim(adjustl(wtimincLine))//char(0))
    endif
