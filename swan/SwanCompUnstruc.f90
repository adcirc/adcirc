...
!Casey 090216: Allow Swan to pass the entire spectrum at once.
    real(SZ), allocatable :: ac2loc2(:,:,:)
...
!Casey 090216: Allow Swan to pass the entire spectrum at once.
    allocate(ac2loc2(MDC,MSC,nverts))
    !
...
!Casey 080827: Add call to make reflective boundaries.
               if ( .FALSE. ) then
                  ...
                  if ( vert(ivert)%atti(VMARKER)==1 .and. &
                     ...
                     !
                     call MakeBoundariesReflective( ivert, ac2, anybin )
                     !
                  endif
                  !
               endif
