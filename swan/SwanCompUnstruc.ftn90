...
!TIMG                      call SWTSTA(136)
                      if ( NUMOBS > 0 ) then
                         ...
                         if ( link(1)/=0 .or. link(2)/=0 ) then
                            !
                            call SWTRCF ( compda(1,JDP2), compda(1,JWLV2), compda(1,JHS), &
                                          link, obredf, ac2, reflso, dummy ,              &
                                          dummy         , dummy          , cax          , &
                                          cay , rdx   , rdy, anybin, spcsig, spcdir )
                            !
                         endif
                         !
                      endif
...
!ADC                if ( .false. ) then
!ADC                   !
!ADC                   if ( vert(ivert)%atti(VMARKER)==1 .and. vert(ivert)%atti(VBC)==0 ) &
...
!TIMG          call SWTSTA(102)
          if ( PNUMS(21) == 0. ) then
             ...
             !
             call SwanConvStopc ( accur, hscurr, hsprev, hsdifc, tmcurr, tmprev, tmdifc, compda(1,JDHS), compda(1,JDTM), &
