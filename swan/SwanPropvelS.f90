    ...
    if ( IREFR /= 0 ) then
       ...
       do is = 1, MSC
          ...
          do iddum = iddlow-1, iddtop+1
             ...
             if ( IDIFFR /= 0 ) cad(id,is) = cad(id,is)*DIFPARAM(iv1) - &
                                             DIFPARDX(iv1)*cgo(is,1)*esin(id) + &
                                             DIFPARDY(iv1)*cgo(is,1)*ecos(id)
             if ( ICUR   /= 0 ) cad(id,is) = cad(id,is) + sincos(id)*(duxdx-duydy) + &
                                             sinsin(id)*duydx - coscos(id)*duxdy
             !
          enddo
          !
       enddo
