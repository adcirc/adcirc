...
!Casey 080213: Make sure that radiation stresses are up-to-date.
      IF(IT.NE.MTC)THEN
         CALL ComputeRadiationStresses
      ENDIF
!
!Casey 080115: These lines are processed only at the last time step.
      IF(IT.EQ.MTC)THEN
!
!Casey 081125: Force Swan to write a hot-start file.
!     CALL BACKUP(AC2,SPCSIG,SPCDIR,KGRPNT,XCUGRD,YCUGRD)
!
!TIMG      CALL SWTSTO(1)                                                      40.23
!
!NADC      DO IUNIT=1,HIOPEN                                                   34.01
!NADC        INQUIRE(UNIT=IUNIT,OPENED=LOPEN)                                  34.01
!NADC        IF (LOPEN.AND.IUNIT.NE.PRINTF) CLOSE(IUNIT)                       40.30
!NADC      END DO                                                              34.01
...
!
!NADC      INQUIRE(UNIT=PRINTF,OPENED=LOPEN)                                   40.30
!NADC      IF (LOPEN) CLOSE(PRINTF)                                            40.30
!
...
!              the external file.
           IF((IGR1.EQ.7).AND.COUPWLV)THEN
              DO INDX=1,nverts
                 ARR(INDX) = (1.0 - InterpoWeight) * ADCIRC_ETA2(INDX,1)
     &                  + InterpoWeight * ADCIRC_ETA2(INDX,2)
              ENDDO
           ELSEIF((IGR1.EQ.2).AND.COUPCUR)THEN
              DO INDX=1,nverts
                 ARR(INDX) = (1.0 - InterpoWeight) * ADCIRC_UU2(INDX,1)
     &                  + InterpoWeight * ADCIRC_UU2(INDX,2)
              ENDDO
           ELSEIF((IGR1.EQ.5).AND.COUPWIND)THEN
              DO INDX=1,nverts
                 ARR(INDX) = (1.0 - InterpoWeight) * ADCIRC_WX2(INDX,1)
     &                  + InterpoWeight * ADCIRC_WX2(INDX,2)
              ENDDO
           ELSE
...
!Casey 080116: Added these lines to grab the y-components.
             IF((IGR2.EQ.3).AND.COUPCUR)THEN
               DO INDX=1,nverts
                 ARR2(INDX) = (1.0 - InterpoWeight) * ADCIRC_VV2(INDX,1)
     &                   + InterpoWeight * ADCIRC_VV2(INDX,2)
               ENDDO
             ELSEIF((IGR2.EQ.6).AND.COUPWIND)THEN
               DO INDX=1,nverts
                 ARR2(INDX) = (1.0 - InterpoWeight) * ADCIRC_WY2(INDX,1)
     &                   + InterpoWeight * ADCIRC_WY2(INDX,2)
               ENDDO
             ELSE
