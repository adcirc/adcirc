...
module ESM
  ...
  subroutine SetModelServices(driver, rc)
    ...
    integer                       :: adc_pet_num,ww3_pet_num,atm_pet_num
   ...
   ! read time hwrf data time interval info
    ...
    call ESMF_ConfigGetAttribute(cf, syear,  label="start_year:",  default=2015, rc=rc)
    call ESMF_ConfigGetAttribute(cf, smonth, label="start_month:", default=12, rc=rc)
    call ESMF_ConfigGetAttribute(cf, sday,   label="start_day:",   default=14, rc=rc)
    ...
    ! read stop time info
    call ESMF_ConfigGetAttribute(cf, eyear,  label="stop_year:",  default=2015, rc=rc)
    call ESMF_ConfigGetAttribute(cf, emonth, label="stop_month:", default=12, rc=rc)
    call ESMF_ConfigGetAttribute(cf, eday,   label="stop_day:",   default=15, rc=rc)
    call ESMF_ConfigGetAttribute(cf, ehour,  label="stop_hour:",  default=0 , rc=rc)
...
!!#define WITHPETLISTS_on
    ...
    if ((adc_pet_num+ww3_pet_num+atm_pet_num).ne.petCount) then
      Stop "petCount does not match (adc_pet_num+ww3_pet_num+atm_pet_num) from Config.rc file!"
    Endif
#endif
