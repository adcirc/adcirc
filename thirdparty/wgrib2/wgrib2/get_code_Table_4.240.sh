...
cat $tmp.1 | sed -e 's=<\([a-zA-Z]*\) [^>]*>=<\1>=g' -e 's=</body>.*==' -e 's=.*<body>==' \
    -e 's=<sub>==g' -e 's=</sub>==g' -e 's=&#961=rho=g'  -e 's=&#948=delta=g' \
...
#  cat $tmp.5 | sed -e 's/ *$//' | sed 's/  */ /g' | sed 's/ (see note.*//' |\
#   sed 's/<td>\(.*\)<td>\(.*\)<td>.*/    case \1: string=\"\2\"; break;/' >$tmp.6

cat $tmp.5 | sed -e 's/ *$//' | sed 's/  */ /g' | sed 's/ ([sS]ee Note.*//' |\
