...
!     M O D U L E    A S Y M M E T R I C  V O R T E X  P R O C E S S I N G

!-----------------------------------------------------------------------
    module asymmetricVortexProcessing
    use sizes, only : sz
    use global, only : wvnx2, wvny2, prn2, &
    nhy, g, rhowat0, &
...
! Import asymmetric hurricane vortex class
    use mesh, only : np, ne, nm, slam, sfea, ics, slam0, sfea0
    USE vortex
    ...
    real(sz) :: vmax_pseudo
    real(sz), dimension(4) ::rmaxw_temp
    real(sz), dimension(4) ::vmaxesbl_temp
...
!     jgf49.30 Check to see if the fort.22 file is there and if not, emit error
!     message and terminate.
    ...
    ! check to see if this is a new cycle; if so, set the cycle time
        if (ient /= 1) then
            if ( iFcstInc(ient) /= iFcstInc(ient-1)) then
...
! set background pressure
    ...
    ! check/set central pressure
        ...
        if ( CycleTime(ient) == CycleTime(1) ) then
        ! we will come back to the first cycle at the end
            cycle
        endif
        if ( CycleTime(ient) == CycleTime(ient-1) ) then
...
!----------------------------------------------
    ...
    ! fill missing values based on how many are missing
        ...
        where (ir(ient,:) == 0) irad(ient,:) = 0.5 * radii_sum
            case(2) ! set missing to half the avg of the 2 radii that are given
            where (ir(ient,:) == 0) irad(ient,:) = 0.5 * radii_sum * 0.5
            ...
            !        jgf49.30: The isotach wind speed can sometimes be zero in cases
            !        where all radii are zero (this has been observed in the BEST
            !        track file for IGOR2010). Including this possibility in the if
            !        statement, so that we can avoid setting the quadrant Rmax to zero
            !        if ivr was zero.
                DO i=1,4
                    if ( (ivr(ient) /= 34) .AND. (ivr(ient) /= 0) .AND. &
                    (ir(ient,i) == 0) ) then
                        rmaxw(ient,i) = 0.0
                    endif
                END DO
            ...
            DO icyc = 1, num_cycles
                ...
                ! fill missing values based on how many are missing
                        ...
                        ! re-calculate quadrantVr at those violated quadrants
                            ...
                            do i = 1, 4
                                ...
                                ! that is not always available.
                                    ...
                                    ! amma(i) = max(min(gamma(i),1.0d0),0.0d0)
                    ...
                    !------------------------------------
                        ...
                        do ient=1,num_entry
                            if ((ivr(ient) == 34) .OR. (ivr(ient) == 0)) then
                                quadflag(ient,:)=1
                            else
                                quadflag(ient,:)=0
                            endif
                        ...
                        do icyc=1,num_cycles
                            last_entry = sum(isotachs_per_cycle(1:icyc))
                            first_entry = last_entry-(isotachs_per_cycle(icyc)-1)
                            if (isotachs_per_cycle(icyc) == 1) then
                                quadflag(last_entry,:)=1
                            else
                            ! loop over quadrants
                                do i=1,4
                                    num_nonzero = &
                                    count(quadflag(first_entry:last_entry,i) /= 0)
                                    select case(num_nonzero)
                    ...
                    ! persist last good 34kt Rmax values if all radii are missing
                        do icyc=1,num_cycles
                            if (isotachs_per_cycle(icyc) == 1) then
                                ...
                                if ((all(ir(ient,:) == 0)) .AND. (atcfRMW(ient) == 0)) then
                                    if ((icyc-1) >= 1) then
                                        rmaxw(ient,:) = &
                    ...
                    !     M O D U L E    D I A G N O S T I C   O U T P U T
                    
                    !-----------------------------------------------------------------------
                        module diagnosticOutput

                        use sizes, only : sz, globaldir, mnp
                        use global, only : wvnx2, wvny2, prn2, nws, &
                    ...
                    !     Variables for recording domain extents
                    ...
                    ! so that we can conveniently loop through them
                        if ((abs(nws) == 12) .AND. (owi_output_factor > 1)) then
                    ...
                    ! values that correspond to each cycle
                        if ((abs(nws) == 19) .OR. (abs(nws) == 20)) then
                            ...
                            icyc=2
                            do ient=2,num_entry
                                if ( cycle_num(ient) /= cycle_num(ient-1) ) then
                    ...
                    ! if we are analyzing just the cycle times
                        if ( defaultOutputIncrement.eqv. .TRUE. ) then
                            ...
                            if ((abs(nws) == 19) .OR. (abs(nws) == 20) .OR. (abs(nws) == 12)) then
                        ...
                        !         output_increment not activated for nws12, since a moving grid is used 
                            ...
                            elseif  ((abs(nws) == 15) .OR. (abs(nws) == 12)) then 
                            ...
                            if (abs(nws) == 12) then
                                ...
                                do iout=2,num_outputs
                                    ...
                                    if ((abs(nws) == 19) .OR. (abs(nws) == 20)) then
                                        if ( outputSecs(iout) > cycleSecs(icyc) ) then
                                            icyc = icyc + 1
                                        endif
                    ...
                    !-------------------------------------------------------------
                        ...
                        if (abs(nws) == 12) then  ! Jie 2013/09
                    ...
                    ! write storm center locations to a file
                        ...
                        ! just skip to the next one
                            if ( (frame /= 0) .AND. (iout /= frame) ) then
                                cycle
                            endif
                            if ( abs(nws) == 15 ) then
                    ...
                    ! will be written
                        ...
                        write(555,'(A)') 'ASCII'
                    ! rite(555,'(A)') 'DATASET STRUCTURED_GRID'
                        write(555,'(A)') 'DATASET UNSTRUCTURED_GRID'
                    ! rite(555,'(A,I3,I3,I3)') 'DIMENSIONS 360 400 1'
                    ...
                    ! save coordinates as Cartesian (m) for use in VTK
                        ...
                        do i=1,np
                            ...
                            if ( dy < ymin ) ymin=dy
                        enddo
                    ! rite(*,*) "DEBUG: xmin=",xmin," xmax=",xmax
                    ! rite(*,*) "DEBUG: ymin=",ymin," ymax=",ymax
            ...
            !     jgf49.30: New variables to support command line options
            ...
            !     jgf49.30 Initialize to reasonable defaults.
