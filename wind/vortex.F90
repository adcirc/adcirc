...
!-----------------------------
    ...
    ! coriolis (and is appropriate in the vicinity of Rmax)
        ...
        if ( (root < 0.d0) .OR. (vicinity <= 0.1d0)) then
            r1 = innerRadius
            r2 = outerRadius
            dr = 1.d0
            DO iter = 1, itmax
...
!-----------------------------
        ...
        ! root and re-evaluate Bs
            ...
            *RhoAir*EXP(Phi_new)/(Phi_new*(Pn-Pc)* mb2pa)
            do ibcont = 1, cont
                ...
                B_new = ((Vmax*kt2ms)**2+Vmax*kt2ms*root*nm2m*corio) &
                *RhoAir*EXP(Phi_new)/(Phi_new*(Pn-Pc)* mb2pa)
                if (abs(B_new-B_new1) <= 0.01d0) then
                    EXIT
                endif
            end do
        !  debug with aswip
    ...
    ! coriolis (and is appropriate in the vicinity of Rmax)
    ! Jie 2013.01
    ! icinity = abs(root-radius(quad))/root
        if (norootflg==1) then
            write(*,*) &
            "iquad=",n, &
...
!=================================================================
    ...
    if (angle <= 45.0d0) then
        iquad = 5
        delta_angle = 45.d0 + angle
    else if (angle <= 135.d0) then
        iquad = 2
        delta_angle = angle - 45.d0
    else if (angle <= 225.d0) then
        iquad = 3
        delta_angle = angle - 135.d0
    else if (angle <= 315.d0) then
        iquad = 4
...
! nearest neighbor weighted interpolation
    if ( delta_angle < 1.d0 ) then
        spInterp=interpR(param, iquad, dist)
    else if (delta_angle > 89.d0) then
